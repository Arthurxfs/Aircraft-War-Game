...
# ==============================================================================
                ...
                # IMPLEMENTATION
                # ---- start your code ---- #
                real_message = msg['message']
                ctime = time.strftime('%d.%m.%y,%H:%M', time.localtime())
                m_t = ctime+' '+real_message
                self.indices[from_name].add_msg_and_index(m_t)
                msg = json.dumps(
                        {"action": "exchange", "from": from_name, "message": real_message})
                # ---- end of your code --- #
...
# ==============================================================================
                ...
                for name in self.indices:
                    searchs = self.indices[name].search(target)
                    if len(searchs) > 0:
                        for i in searchs:
                            search_rslt += '('+name+')'+i[1]+'\n'
            
                
                # ---- end of your code --- #
                mysend(from_sock, json.dumps(
                    {"action": "search", "results": search_rslt}))
            elif msg["action"] == "gamescore":
                list_score = []
                list_result = [] 
       
                from_score = str(msg["target"])
                from_name = self.logged_sock2name[from_sock]
                the_guys = self.group.list_me(from_name)
                print(from_name + " gets " + from_score)
       
                self.dic_score[from_score] = from_name

                for k, v in self.dic_score.items():
                    list_score.append(int(k))

                list_score.sort()
                list_score.reverse()
                for i in list_score:
                    result = self.dic_score[str(i)] + " has got " + str(i) + " points"
                    list_result.append(result)
       
                for g in the_guys:
                    to_sock = self.logged_name2sock[g]
                    mysend(to_sock, json.dumps({"action":"gamescore", "target":list_result}))
                ...
                msg = json.dumps(
                        {"action": "accept", "from": from_name})
                mysend(to_sock, msg)
            elif msg["action"] == "refuse":
                from_name = msg['from']
                to_sock = self.logged_name2sock[msg['to']]
                msg = json.dumps(
                        {"action": "refuse", "from": from_name})
                mysend(to_sock, msg)
